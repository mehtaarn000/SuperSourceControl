...
func readBranchLog(name string) ([]string, error) {
	if !validateBranchName(name) {
		return nil, fmt.Errorf("invalid branch name: %q", name)
	}
...
func currentBranch() (string, error) {
	...
	name := string(data)
	if !validateBranchName(name) {
		return "", fmt.Errorf("invalid current branch: %q", name)
	}
	return name, nil
}
...
func createBranch(name string) error {
	...
	if len(commits) == 0 {
		return fmt.Errorf("make at least one commit before creating a branch")
	}
...
func switchBranch(name string) error {
	...
	if len(commits) == 0 {
		return fmt.Errorf("branch %q has no commits", name)
	}
	...
	if len(oldCommits) == 0 || oldCommits[len(oldCommits)-1] != tip {
		if err := restoreSnapshot(tip); err != nil {
			return err
		}
	}
...
func checkBranchDeletion(name string) error {
	if _, err := readBranchLog(name); err != nil {
		return err
	}
	current, err := currentBranch()
	if err != nil {
